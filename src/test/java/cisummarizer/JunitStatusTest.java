...
public class JunitStatusTest {
  ...
  private static final Path RESOURCE_DIR = Paths.get("src/test/resources/junit-status");

  private Status status;

  void getStatusFor(String fileName) {
    Path pathToFile = RESOURCE_DIR.resolve(fileName);
    status = new JunitStatus(pathToFile);
  }

  @Test
  @DisplayName("a status has a list of errors if the test result file isn't there")
